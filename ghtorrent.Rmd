...
![The MySQL schema](schema.png){ width=30% }
...
Which year did RxJS get most stars from Argentina?

**Time**: 3 mins

**Prize**: A copy of Beautiful Architectures signed by the authors!

![Projects and commits tables](ba.png){ width=30% }

## Challenge results

```{sql, connection=db, echo=T}
select substr(w.created_at, 1, 4) as year, count(substr(w.created_at, 1, 4)) as count
from watchers w left join users u on u.id = w.user_id 
where u.country_code = 'ar'
group by year
```
